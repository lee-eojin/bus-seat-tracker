...
import { isNonBoardingStop, type RouteCache } from '../../../packages/domain/src/model.js';
import {
  applyNetDemand,
...
} from '../../../packages/domain/src/profile.js';
import { boardingVerdict, expectedDemandAt, nextFullDepartureStreak, queueStatement, verdictSeatMargin, type BoardingVerdict } from '../../../packages/domain/src/boarding.js';
import { loadRouteCaches, loadSnapshots } from './data-source.js';

// 좌석 관측만으로 정류장 대기 인원을 복원한다
// (docs/research/2026-07-26-to-2026-08-02-queue-recovery.md).
//
...
// 도착률이 음수로 나온다 (대기열 복원 연구 기록 §7 구현 주의).
...
// 태우므로 분포 전파가 이 제약을 지켜 준다 (대기열 복원 연구 기록 §8).
...
async function main(): Promise<void> {
  ...
  if (options.allStops && options.byDay) {
    // 정류장×날짜 λ. 날짜 간 분산 측정이 예측기 승격의 선결 조건이다 (대기열 복원 연구 기록 §16).
