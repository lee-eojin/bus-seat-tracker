// 수집 스케줄. 워크플로와 예산 계산기(tools/collector/scripts/call-budget.mjs)가
// 같은 정의를 쓴다. 예전에는 이 로직이 워크플로 안 인라인 JS로만 있어 테스트도 검산도
// 불가능했고, 배포 문서의 호출 예산 표를 손으로 세다가 실제와 어긋났다.
//
...
// (docs/research/2026-07-26-to-2026-08-02-queue-recovery.md §9). 10분 간격에서는 버스가 관측 사이에 5~7정류장을 지나
// 정류장 단위 승차량을 읽을 수 없다. 배치 근거는 대기열 복원 연구와 좌석 예보 검증의
// 필드 기록 시각이다. 아침 절정 07:00-08:00과 범계역 기록(08:37), 저녁 절정 18:00-18:30과
// 판교역 기록(18:57)이 모두 안에 들어온다. 전 피크를 60초로 돌리면 API 한도에 육박한다.
//
// 낮 창은 매시 1회 크론이 남기던 공백을 메운다. 60분 간격 관측은 순수요 학습의 운행
// 분할 기준(packages/domain/src/profile.ts runSplitGapMs 45분)에 전부 걸려 관측쌍을 하나도 만들지
