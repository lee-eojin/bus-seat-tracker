...
| 경로 | 내용 | 공개 여부 |
|---|---|---|
| `data/routes/*.json` | 노선·정류장 캐시 | 비공개 저장소만 |
...
| `scorecard.json` | 날짜별·누적 집계 지표 | Actions 아티팩트 |
...
| `pairs` | 같은 차량 운행에서 시작·하류 관측을 대조한 수 |
...
| 최신 폴드 MAE·Brier가 기준선 이상 | 경고 | 하루 표본은 작아 누적 판정을 바로 뒤집지 않음 |
...
| `unknownBasis` | 예측의 출발 정류장·좌석과 맞는 근거 관측을 찾지 못한 수 |
...
4. 프로파일·귀속 규칙을 바꿨다면 화면 빌드와 백테스트가 같은 구현을 쓰는지 확인한다.
