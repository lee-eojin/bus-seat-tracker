...
- **대기 인원 복원**: 좌석을 남기고 떠난 버스는 그 순간 줄이 비었다는 뜻이다. 이 해소 사건들 사이의 보존식으로 도착률을 식별한다 (`docs/04-queue-recovery.md`)

### 검증된 성과 (`docs/05-validation-2026-07-24.md`)
...
다만 **대기 인원 예측기는 아직 서비스에 쓰지 않는다.** 도착률의 날짜 간 분산을 측정하지 못했다. 사후 재구성만 가능하다 (`docs/04-queue-recovery.md` §7, §11).
